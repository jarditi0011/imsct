...
tumors<-names(table(unlist(str_split(imsct$Diseases, ",|;"))))
...
countchrmentstumor<-function(region){
  objs<-unlist(str_split(region, "(?=.$)"))
  genes<-unique(annot5$V1[annot5$V2==objs[1] & annot5$V6==objs[2]])
  df<-genesonly$Diseases[genesonly$Gene %in% genes]
  df<-sapply(tumors, function(tumor){str_count(df, tumor)}, 
...
ments<-t(data.frame(sapply(unique(paste0(annot5$V2, annot5$V6)), countchrmentstumor, simplify=F, USE.NAMES = T)))
colnames(ments)<-c("Astrocytoma", "Diffuse Midline Glioma", "Ependymoma", 
...
names(test)<-c("Region", "Key", "Count")
Value<-apply(test,1,function(x){if(x[2]=="Total Mentioned Genes"){"Genes"}else{"Tumor"}})
test<-cbind(test, Value)
test$Region<-factor(test$Region, levels=myorderx)

colors=c('#000000', '#A03F3F', '#A05E3F', '#A0843F', '#7EA03F', '#3FA049', '#3F87A0', '#4F3FA0', '#753FA0', '#A03FA0')

{
  p<-ggplot(data=test, aes(x=Value, y=Count, colour=Key, fill=Key)) +
    ...
    labs(title="Tumor Markers in Chromosomal Regions") +
...
ggsave(filename=paste("tumorByRegion.jpeg"), 
       plot=p,
       device ="jpeg")
